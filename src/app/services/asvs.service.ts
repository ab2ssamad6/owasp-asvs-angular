...
export class AsvsService {
  ...
  readonly allItems = computed<MissingItem[]>(() =>
    ASVS_DATA.flatMap(cat =>
      cat.items.map(item => ({ ...item, catId: cat.id, catTitle: cat.title }))
    )
  );
  ...
  readonly checkedCount = computed(() =>
    Object.values(this.checkedItems()).filter(Boolean).length
  );
  ...
  readonly categoryStats = computed<Record<string, CategoryStats>>(() => {
    ...
    return ASVS_DATA.reduce((acc, cat) => {
  ...
  readonly filteredItems = computed<AsvsItem[]>(() =>
    this.visibleCategories().flatMap(c => c.items)
  );
  ...
  selectAllVisible(): void {
    ...
    this.filteredItems().forEach(i => { updates[i.id] = true; });
    this.checkedItems.update(prev => ({ ...prev, ...updates }));
  }

  clearAllVisible(): void {
    const updates: Record<string, boolean> = {};
    this.filteredItems().forEach(i => { updates[i.id] = false; });
    this.checkedItems.update(prev => ({ ...prev, ...updates }));
  }
}
